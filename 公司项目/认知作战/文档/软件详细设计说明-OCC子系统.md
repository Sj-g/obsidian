...
本文件为「认知作战平台」（以下简称"系统"或"平台"）舆情作战业务子系统（OCC，软部件标识 M-OCC-00）的软件详细设计说明（SDD）。它是《软件需求规格说明-OCC子系统》V3.8 功能需求 R-OCC-001 ~ R-OCC-009（共 9 项）与《软件概要设计说明》V2.6 模块 M-OCC-01 ~ M-OCC-09（共 9 模块 / 31 功能点）的详细设计下沉，描述 OCC 子系统各模块的内部结构、类与接口、数据结构、算法、状态机与部署单元，作为编码实现与单元测试的依据。
...
| 文件 | 说明 |
| --- | --- |
| 《软件需求规格说明-OCC子系统》V3.8 | OCC 功能需求 R-OCC-001 ~ R-OCC-009、非功能需求、外部接口 EI-04/05、内部接口 II-01/01a/10/12/12a/13/15/18 的直接输入 |
...
occ-service 为 Java 17 / Spring Boot 3.x 单一微服务，八模块为同进程内 Java 包。鉴权统一经 com-auth-lib 本地验签 + `org_id` 行级隔离（NR-S-01 / NR-C-01），与 OM / COM / MC mc-service 同栈。八模块覆盖作战指挥流（目标档案 → 方案规划 → 编排引擎）、效果闭环流（评估 → 策略迭代）、内容管理流（主数据 → 审核）三类业务逻辑。
...
- **TargetEventPublisher**：变更经 Kafka `occ-target-changed` 广播 `target.changed`（F-OCC-01-05），作战编排 / 效果评估订阅响应。
...
#### 5.1.3 M-OCC-04 作战方案规划（R-OCC-006，2 功能点）
...
- **动作收口**：所有节点动作经 MC 统一执行网关落地（CON-07），认知作战账号养号 / 投放经移动端 ADB / 无障碍通道，爬虫账号相关经桌面端 CDP 通道。
...
- **EntitySearchService**：实体检索与档案（F-OCC-02-03）。按账号 / 昵称 / 标识经 Elasticsearch 检索（检索超时降级返回部分结果）；聚合多来源信息形成全维度档案视图（供 M-OCC-01 融合 + 经 II-18 供 SWM R-SWM-005 用户画像）。
...
#### 6.2.3 II-10 作战编排下发与执行回传（OCC ↔ MC，★ 核心，节点级协议）
...
#### 6.2.8 II-18 用户画像查询（OCC → SWM）

OCC M-OCC-02 产出的全维度档案（R-OCC-004）作为用户画像，供 SWM R-SWM-005 提示词自动生成。REST 查询：SWM 按 `account_id` / 目标标识请求 → OCC 返回全维度档案视图（经 `EntitySearchService.profile()`）。

#### 6.2.9 内部协同：occ-service ↔ occ-analysis（双部署单元）
...
- OCC 编排落地的所有节点动作经 MC 统一执行网关（R-MC-005）收口，OCC **不直接操作终端**；认知作战账号养号 / 投放经移动端 ADB / 无障碍通道，爬虫账号相关经桌面端 CDP 通道。
...
- **权限**：`@RequirePermission` 注解决策，按角色（情报分析人员 / 作战指挥 / 策略工程师 / 内容运营 / 审核人员）控制目标建档 / 编排启动 / 策略迭代 / 内容发布审核等动作。
...
flowchart TB
    ...
    subgraph K8S["Kubernetes 集群"]
        ...
        subgraph SRC_NS["各子系统 Namespace"]
            SS["dc-service / mc-service / swm-service /<br/>com-service / om-service<br/>（II-01/01a/10/12/12a/13/15/18 交互）"]
        end
...
| R-OCC-006 作战编排 | M-OCC-04 / F-OCC-04-01~02、M-OCC-05 / F-OCC-05-01~03 | §5.1.3、§5.1.4 |
...
非功能需求覆盖：NR-C-01（§7.4 org_id 行级隔离）、NR-C-02（§7.4 最小权限）、NR-C-04（§7.4 导出审计留痕）、NR-R-01（§3.2 当前单副本，V2 演进多副本）、NR-M-01（§3.2 子系统可独立部署升级）、NR-M-02（§8.2 关键策略可配置）、NR-M-03/04（§8.3 可观测）、NR-F-03（§7.2 编排引用账号受账号异常停止约束）。约束覆盖：CON-04（OCC 作战闭环核心链路优先实现）、CON-06（§3.1 私有化，IRS 本地大模型）、CON-07（§7.2 编排落地经 MC 执行网关收口）、CON-11（§5.1.7 内容主数据权威源、§6.2.4 II-12 使用方引用标识）、CON-13（§7.2 认知作战账号经移动端 ADB / 无障碍通道）。
