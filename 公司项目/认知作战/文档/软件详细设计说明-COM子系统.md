...
本文件为「认知行动平台」（以下简称"系统"或"平台"）公共功能子系统（COM，软部件标识 M-COM-00）的软件详细设计说明（SDD）。它是《软件需求规格说明-COM子系统》V3.8 功能需求 R-COM-001 与《软件概要设计说明》V2.5 模块 M-COM-01/02 的详细设计下沉，描述 COM 子系统各模块的内部结构、类与接口、数据结构、算法、状态机与部署单元，作为编码实现与单元测试的依据。
...
> V1.1（ADR-004）定位补充：COM 系统管理（用户/角色/组织/认证）现为**独立的「系统管理」控制台**（原与 OM 合用「运维管理控制台」，ADR-004 删除 OM 前端运维监控页 OM-01~05 后，COM 系统管理页 OM-06~10 独立成纯 COM 控制台，控制台品牌名「运维管理」→「系统管理」，顶栏代号「OM」→「系统」，仅超级管理员可访问）。本文件 V1.1 仅补充控制台归属与命名说明，COM 子系统的模块/接口/数据/约束设计**不变**。

COM 的认证域只覆盖「**人登录认知行动平台本身**」（6 类操作者角色）；不覆盖 MC（R-MC-011/M-MC-11）维护的「**机器人在目标平台上的作业账号主数据**」（爬虫/智能体账号的凭据、平台、生命周期、风险）。两者是完全独立的身份领域，本文件不涉及后者。
...
| 《软件详细设计说明-OM子系统.md》V1.2 | Java/Spring Boot 同栈微服务的形态参照、PostgreSQL/Redis 复用约定；V1.2（ADR-004）删 OM 前端后 COM 独立为系统管理控制台的关联依据 |
