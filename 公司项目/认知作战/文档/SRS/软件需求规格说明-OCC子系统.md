...
本文件为「认知作战平台」舆情作战业务子系统（OCC，OCC（Opinion Combat Command））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.8）按子系统拆分的组成部分，承载 舆情作战业务子系统 的全部功能需求（R-OCC-001 ~ R-OCC-009（9 项））。需求编号 `R-OCC-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。

### 1.2 文档概述

本分册章节安排：第 1 章引言；第 2 章引用文件；第 3 章 舆情作战业务子系统 功能需求（按子区域给出各 `R-OCC-NNN` 需求块）。系统级共性内容（引言概述、术语缩略语、接口 EI/II、数据 DR、非功能 NR、约束 CON、合格性规定、附录）见《软件需求规格说明-总册》。
...
| 《软件概要设计说明》V2.7 | OCC 子系统功能模块设计（M-OCC-NN） |
| 《软件详细设计说明-OCC子系统》 | OCC 子系统详细设计 |

---

## 3 舆情作战业务子系统（OCC）功能需求

> 本节为 舆情作战业务子系统（OCC）的全部功能需求，对应 R-OCC-001 ~ R-OCC-009（9 项）。需求编号 `R-OCC-NNN` 不变，逐字搬迁自原《软件需求规格说明》。
...
**关联接口**：II-13 分析数据与情报接口（入，从 DC 取原始舆情/传播数据）；II-18 用户画像查询接口（出，向 SWM 提示词自动生成 R-SWM-005 提供用户画像）。
