...
本文件为「认知行动平台」私有化大模型部署（IRS，IRS（Inference Service））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.9）按子系统拆分的组成部分，承载 私有化大模型部署 的全部功能需求（R-IRS-001（1 项））。需求编号 `R-IRS-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
