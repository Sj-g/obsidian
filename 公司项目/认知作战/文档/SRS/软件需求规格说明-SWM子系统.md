...
本文件为「认知行动平台」蜂群智能体子系统（SWM，SWM（Swarm））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.9）按子系统拆分的组成部分，承载 蜂群智能体子系统 的全部功能需求（R-SWM-001 ~ R-SWM-005（5 项））。需求编号 `R-SWM-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
...
**需求目的**：根据提示词（人设属性直接写进提示词文本）构建可作业的智能体——配置驱动作业的提示词模板与作业策略（目标、场景、边界），组装为提示词包（提示词模板、账号级动态提示词实例、作业策略）并以 agent_id（模板共性）+ account_id（动态实例个性）组合为标识，经内部接口 II-14 同步至群控子系统（MC）持存、供舆情行动子系统（OCC）行动编排引用。一个 agent_id 可绑定 N 个 account_id，每个账号拥有独立的动态提示词实例。记忆初值不在提示词包内，由 SWM 记忆服务（R-SWM-002，按 account_id 索引）独立承载。本功能确立"SWM 生成提示词包、MC 持存并运行（agent-runtime）、OCC 编排引用"的分工。
