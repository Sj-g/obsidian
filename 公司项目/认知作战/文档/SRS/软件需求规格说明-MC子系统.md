...
本文件为「认知作战平台」多设备矩阵自动化群控子系统（MC，MC（Multi-device Control））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.8）按子系统拆分的组成部分，承载 多设备矩阵自动化群控子系统 的全部功能需求（R-MC-001 ~ R-MC-014（14 项））。需求编号 `R-MC-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
...
**需求目的**：维护账号与智能体（agent_id）的严格 1:1 绑定关系，叠加账号-终端-代理绑定规则形成四元作战单元（agent_id:account_id:终端:代理 IP = 1:1:1:1）；并在账号被封禁后支持运营人员发起资产迁移，将原 agent_id 的人设标签、提示词版本与记忆整体继承到新账号，保证人格连贯性。
...
| 规则 | 说明 |
|------|------|
| 1:1 绑定 | 账号与智能体（agent_id）严格 1:1 绑定，由本子系统记录并维护 agent_id↔account_id 绑定关系 |
...
**用户需求概述**：作为智能体的执行宿主，接收并持存 SWM 同步来的提示词包（以 agent_id 为标识），在任务执行时由 agent-runtime 子模块运行（场景适配、动态调用提示词、读记忆、调 IRS 推理）驱动设备作业，并在执行前校验人设一致性，作为最后一道闸。

---

**第①步 目标澄清**（工具：干系人多角度分析 ｜ 产物：子目标清单 ｜ 验收：是否穷举所有干系人的价值诉求）：①MC 能持存 SWM 同步来的提示词包并作为 OCC 编排节点来源；②执行前校验人设一致性（最后一道闸）；③执行时由 agent-runtime 运行提示词包、调用 IRS 推理并经网关落地。
...
| 角色 | 操作 |
|------|------|
| 系统（接收同步） | 经 II-14 接收 SWM 同步的提示词包（agent_id/人设/提示词/策略/版本）、以 agent_id 为主键持存、提示词包更新时取最新版本、记录四元绑定关系 |
| 系统（执行前校验） | 任务执行前校验"当前 account_id 绑定人设标签"与"agent_id 提示词包人设标签"一致性，作为最后一道闸 |
| 系统（执行调用 agent-runtime） | agent-runtime 读记忆+按提示词包场景适配/动态调用+调 IRS 推理产出动作指令、动作经执行网关落地、作为 OCC 编排节点来源（以 agent_id 引用） |
...
正常流程涉及 4 个子系统/组件：蜂群智能体 SWM（II-14 同步提示词包）、私有化大模型 IRS（EI-05/II-08 推理）、统一执行网关（II-07 落地）、舆情作战 OCC（以 agent_id 引用作智能体节点来源）。
...
sequenceDiagram
  ...
  participant 网关 as 执行网关(II-07)
  participant 终端 as 执行终端
  SWM->>MC: 同步提示词包(agent_id/人设/提示词/策略/版本, II-14)
  MC->>MC: 以agent_id持存+记录四元绑定
  MC-->>SWM: 同步确认+持存版本号
  OCC->>MC: 作战编排引用agent_id(智能体节点)
  MC->>MC: 任务执行前人设一致性校验
  alt 人设一致
    MC->>记忆: 读取该agent_id记忆(R-SWM-002)
    记忆-->>MC: 历史/场景记忆
...
| 规则 | 可测的验证点 | 归属子目标 |
|------|------------|-----------|
| 以 agent_id 为主键持存 SWM 同步的提示词包 | 给定 agent_id 可取回持存提示词包 | ①持存提示词包 |
| 记录 agent_id↔account_id↔终端↔代理 IP 四元绑定 | 四元绑定关系可记录与查询 | ①持存提示词包 |
...
| 作为 OCC 作战编排智能体节点来源，以 agent_id 引用 | OCC 编排中的智能体节点可引用 MC 持存提示词包 | ①持存提示词包/③调用执行 |
...
**需求目的**：作为智能体的执行宿主，接收并持存蜂群智能体子系统（SWM）同步来的提示词包（以 agent_id 为标识，含人设标签、提示词、作业策略），在任务执行时由 MC 的 agent-runtime 子模块运行（场景适配、动态调用提示词、调用 IRS 推理）驱动设备作业，并在执行前校验人设一致性。本功能确立"SWM 为提示词生成与记忆中心、MC 为智能体执行宿主（含运行时 agent-runtime）"的分工。

**使用角色**：系统（接收同步、持存、校验、调用）。

**前置条件**：SWM 提示词包同步接口（II-14）可用；执行网关可用；智能体已与账号建立 agent_id↔account_id 1:1 绑定（R-MC-013）。

**输入**：来自 SWM 的提示词包（agent_id、人设标签、提示词、作业策略）、任务执行请求（含目标 agent_id 与 account_id）。

**输出**：持存的提示词包（按 agent_id 索引）、四元绑定关系（agent_id↔account_id↔终端↔代理 IP）、智能体执行调用结果、人设一致性校验结果、执行网关的动作日志。

**业务规则**：

| 规则 | 说明 |
|------|------|
| 提示词包持存 | 本子系统以 agent_id 为主键接收 SWM 同步的提示词包并持存，作为任务执行时 agent-runtime 运行智能体的依据 |
...
| 人设一致性校验 | 智能体模式下任务执行前，本子系统校验"当前 account_id 绑定的人设标签"与"agent_id 提示词包中的人设标签"是否一致，一致方可调用 IRS 推理并经执行网关落地动作，不一致则拒绝执行并告警；一致性收口于执行宿主，作为最后一道闸 |
| 执行调用（agent-runtime） | 任务执行时本子系统的 agent-runtime 子模块按提示词包中的提示词与作业策略进行场景适配与动态调用，并从 SWM 记忆服务（R-SWM-002）读取该 agent_id 的记忆注入，再调用 IRS 本地大模型进行视觉推理与规划，产出的动作指令经执行网关落地；agent-runtime 与执行网关逻辑隔离 |
| 定义更新 | 提示词包更新时（SWM 以同一 agent_id 重新同步并升级版本号）本子系统更新持存版本，保证执行使用最新版本 |
| OCC 节点来源 | 本子系统可作为 OCC 作战编排的执行节点来源——编排中的智能体节点即以 agent_id 引用本子系统持存的提示词包 |

**异常场景**：SWM 同步失败时保留上一可用定义并告警；智能体定义缺失时降级为脚本模式或人工介入；推理调用 IRS 失败时降级处理；人设一致性校验不通过时拒绝执行并告警；四元绑定关系缺失或冲突时挂起任务并提示。

**验收标准**：可以 agent_id 接收并持存 SWM 同步的提示词包；四元绑定关系（agent_id↔account_id↔终端↔代理 IP）可记录与查询；任务执行前可校验人设一致性且不一致时拒绝执行；任务执行时可由 agent-runtime 按提示词包运行并经执行网关落地动作；提示词包更新后可使用最新版本执行；可作为 OCC 作战编排的智能体节点来源（以 agent_id 引用）。
