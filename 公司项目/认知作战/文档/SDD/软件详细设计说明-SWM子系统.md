...
本文件为「认知行动平台」（以下简称"系统"或"平台"）蜂群智能体子系统（SWM，软部件标识 M-SWM-00）的软件详细设计说明（SDD）。它是《软件需求规格说明-SWM子系统》V3.9 功能需求 R-SWM-001~005 与《软件概要设计说明》V2.8 模块 M-SWM-01/02/04/05/06/07 的详细设计下沉，描述 SWM 子系统各模块的内部结构、类与接口、数据结构、算法、状态机与部署单元，作为编码实现与单元测试的依据。
