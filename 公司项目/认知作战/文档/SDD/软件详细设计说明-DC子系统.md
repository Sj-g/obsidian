...
本文件为「认知行动平台」（以下简称"系统"或"平台"）数据爬取子系统（DC，软部件标识 M-DC-00）的软件详细设计说明（SDD）。它是《软件需求规格说明-DC子系统》V3.9 功能需求 R-DC-001~008 与《软件概要设计说明》V2.8 模块 M-DC-01~04 的详细设计下沉，描述 DC 子系统各模块的内部结构、类与接口、数据结构、算法、状态机与部署单元，作为编码实现与单元测试的依据。
...
| 文件 | 说明 |
| --- | --- |
| 《软件需求规格说明-DC子系统》V3.9 | DC 功能需求 R-DC-001~008、非功能需求、接口（EI-01/02、II-01/04/05/13/15）、数据 DR-06/08、约束 CON-03/05 的直接输入 |
| 《软件概要设计说明》V2.8 | DC 模块划分 M-DC-01~04、§3.7 环境部署、§4.1 DC 模块组成与关键设计 |
...
DC 子系统技术栈遵循《软件需求规格说明-DC子系统》V3.9 运行环境与 CON-03 约束：
