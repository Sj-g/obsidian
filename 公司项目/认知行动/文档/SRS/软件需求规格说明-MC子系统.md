...
本文件为「认知行动平台」多设备矩阵自动化群控子系统（MC，MC（Multi-device Control））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V4.3）按子系统拆分的组成部分，承载 多设备矩阵自动化群控子系统 的全部功能需求（R-MC-001 ~ R-MC-015（15 项，V4.1/V3.10 新增 R-MC-015 真机 Agent 运行时与编排））。需求编号 `R-MC-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
...
| 《软件概要设计说明》V3.2 | MC 子系统功能模块设计（M-MC-NN） |
...
本子系统是账号资源的平台级权威源，负责注册、验证、养护策略与生命周期等主数据的维护，并经事件总线向数据爬取（DC）、蜂群智能体（SWM）、社信行动（OCC）等使用方共享分发账号标识（account_id）。账号按终端类型遵循差异化的绑定规则：真机、云手机、虚拟化真机出于风控要求执行"一设备一账号一 IP"，指纹浏览器 Profile 执行"一 Profile 一账号一 IP"（浏览器实例可多开）。内容资源由社信行动业务子系统（OCC）统一管理（见 R-OCC-009），本子系统仅作为内容使用方引用内容标识（content_id）进行内容投放。
...
**需求目的**：统一管理四类执行终端（移动端：真机、云手机、ARM 板卡、虚拟化真机；桌面端：指纹浏览器 Profile）的台账维护、分组标签、实时状态查询与批量操作编排，为远控、群控与任务执行提供终端资源池。**V4.1（V3.10）边界调整**：原"Agent 接入注册 + 心跳判定在线"语义经 CON-15 划归 **R-MC-015**（R-MC-015 持 II-02 endpoint，全权管 Agent 生命周期）；本需求回归**纯台账持有方**——订阅 R-MC-015 的设备上下线事件写库与更新状态，桌面端 Profile 创建（调 EI-06）仍归本需求。

**使用角色**：设备运维人员（注册与维护执行终端）、运营人员（批量操作）。

**前置条件**：R-MC-015 真机 Agent 运行时可用（移动端接入与状态由 R-MC-015 推送）；商用指纹浏览器产品已部署且其 API 可用（EI-06）；对象存储可用（EI-04）。

**输入**：来自 R-MC-015 的设备注册 / 上下线事件、浏览器 Profile 创建请求（分组、标签）、分组与标签规则、针对终端集合的批量操作指令。
...
| 规则         | 说明                                                                                       |
| ---------- | ---------------------------------------------------------------------------------------- |
| 台账维护（移动端）  | 本需求订阅 R-MC-015 推送的设备注册事件，记录设备类型（真机/云手机/ARM 板卡/虚拟化真机，区分虚拟与实体）、版本、型号、分组与标签；状态由 R-MC-015 心跳判定后推送，本需求只持有与查询 |
| Profile 创建 | 浏览器 Profile 经商用指纹浏览器 API（EI-06）创建与配置，启动后由商用产品返回调试端口（CDP），系统记录 Profile 标识、内核版本、调试端口、分组与标签 |
| 状态查询       | 本需求对外提供终端状态查询接口（在线 / 离线、指标）；状态数据由 R-MC-015 推送（移动端）或 EI-06 查询（桌面端），本需求不主动判定                  |
| 批量操作       | 移动端支持批量安装与卸载 App、批量重启、批量截图（动作经 R-MC-005 网关 → R-MC-015 落地）；浏览器 Profile 支持批量创建、启动、关闭、批量截图；均可通过对象存储（EI-04）临时链接进行文件上传与下载 |

**异常场景**：接收到 R-MC-015 推送的离线事件时标记离线并告警；Profile 启动失败时记录错误码并重试；终端指标异常（过热、存储满、内存超限）时隔离并停止派发任务（V1.2 由 R-MC-015 上报指标触发，MVP 暂不做自动隔离）；批量操作部分失败时记录失败项并支持重试。

**验收标准**：可订阅 R-MC-015 事件维护移动端终端台账；浏览器 Profile 可经 EI-06 创建并记台账；执行终端状态可查询；移动端与桌面端的批量操作均可执行并返回结果。

**关联接口**：EI-04 对象存储、EI-06 指纹浏览器服务、II-04 日志指标上报（至 OM）、R-MC-015 设备上下线事件订阅（内部）。
...
| 通道适配 | **V4.1（V3.10）接口/实现分离**：本需求（R-MC-005）只持 `DeviceSdk` 统一接口（click/input/screenshot/install/execute）+ `DeviceChannelRouter` 按 `terminal_type` 路由（对应功能点 F-MC-05-05 收缩为接口与路由薄层）；真机 `RealDeviceSdk` 的实现下沉 **R-MC-015**（M-MC-15.c 经 II-02 下发 Agent 落地），桌面端 `ProfileSdk` 的 CDP 直连实现留 V1.2；移动端 ADB / 无障碍、桌面端 CDP 控制通道对上层透明 |
...
正常流程涉及 4 个子系统/组件：终端 Agent（II-02）、统一执行网关（II-07）、运维监控 OM（II-04）、社信行动 OCC（II-10 效果回传）。可观测性统一收口于 MC，动作留痕是统计复盘与效果回传的同源数据底座（不重复采集）。
...
| 效果回调 | 系统将投送效果数据通过回调机制联动回传至社信行动子系统（OCC），供效果评估闭环使用 |
...
| R-MC-013 智能体账号绑定与资产迁移 | ✅ 聚焦绑定与迁移 | ✅ 1:N 绑定→封号→账号资产迁移延续行为 | ✅ 建立在 010/011 之上 | ✅ 绑定/迁移可验 |
...
正常流程涉及 4 个子系统/组件：蜂群智能体 SWM（II-14 同步提示词包）、私有化大模型 IRS（EI-05/II-08 推理）、统一执行网关（II-07 落地）、社信行动 OCC（以 agent_id 引用作智能体节点来源）。
...
| 行动关系（agent_id:account_id:终端:代理 IP = 1:N:1:1） | 缺失或冲突 | 挂起任务并提示 |
...
### 真机 Agent 运行时与编排（V4.1 / V3.10 新增）

#### R-MC-015 真机 Agent 运行时与编排

**需求目的**：提供真机 Agent 运行时（设备主机侧独立部署单元）+ 中心侧 Agent 编排网关，承载真机设备的连接生命周期（注册 / 心跳 / 占用锁）、真机控制通道（ADB / 无障碍）、远控画面源（scrcpy H.264 喂 mc-sfu）。Agent 是 MC 体系内**唯一触碰真机的进程**（CON-15）；所有真机动作经 R-MC-005 统一执行网关收口，由网关路由到本需求实现的 `RealDeviceSdk`，再经 II-02 长连接下发 Agent 落地。本需求填补原 R-MC-005「设备策略适配」功能点下真机通道实现的空白——把 ddmlib / uiautomator2 / scrcpy / Agent 生命周期从「黑盒甩给设备端程序」正式纳入本子系统设计范围（借鉴 SonicCloudOrg，已 archived，Apache 2.0 fork 自维护）。

**使用角色**：系统（Agent 自治常驻 + 中心侧编排）。

**前置条件**：R-MC-005 执行网关可用（真机动作经网关路由）；真机已 USB 接入 Agent 主机；Agent 主机已安装 Android SDK / ddmlib 依赖；mc-sfu 可达（远控媒体面端口直推）。

**输入**：Agent 启动配置（agentKey、server_url、设备 udId 清单）；来自 R-MC-005 网关下发的动作指令（经 `RealDeviceSdk` 实现）；来自 R-MC-004 远程控制的远控占用请求与 scrcpy 启动协商；Agent 周期心跳。

**输出**：Agent 注册成功事件（推 R-MC-001 写台账）；终端在线 / 离线状态（推 R-MC-001 更新台账）；动作执行结果（回 R-MC-005 网关）；远控画面源（scrcpy H.264 推流到 mc-sfu 指定端口）；Agent 自身日志。

**业务规则**：

| 规则 | 说明 |
|------|------|
| Agent 独立部署单元（CON-15） | 真机 Agent 是独立部署的 Spring Boot 进程，每个插真机的 Linux/Mac 主机一台，**不嵌入 mc-service**；mc-service 重启不影响 Agent 与真机的连接；Agent 可横向扩展到多主机（V1.2 演进，MVP 单主机） |
| 唯一触真机进程 | mc-service 不直接调 ADB / uia2 / scrcpy，所有真机操作经 II-02 由 Agent 落地；本需求模块（M-MC-15）是 MC 体系内唯一触碰真机的进程集合 |
| Agent 生命周期全权管理 | 中心侧编排器（M-MC-15.c）持 II-02 WebSocket endpoint，全权管 Agent 注册 / 鉴权 / 心跳 / 占用锁 / 媒体面协商；R-MC-001 终端台账回归纯持有方（订阅本模块设备上下线事件写库），不再持 endpoint |
| 动作路径穿编排器 | 真机动作路径：R-MC-005 网关 → 本模块中心侧编排器（RealDeviceSdk 实现）→ II-02 → Agent → ADB / 无障碍落地；编排器是动作路径上的必经节点，负责占用锁检查、设备状态校验、媒体面协商等前置 |
| 设备占用锁互斥 | 同一台设备同时只让一个会话占用（远控 / 自动化互斥），MVP 内存锁（单 mc-service 副本），V1.2 演进 Redis 分布式锁 |
| 远控画面源与转发分离 | 本模块只产画面源（scrcpy server 启动 + H.264 产出），经 II-02 协商后 Agent 直接推流到 mc-sfu 指定端口（不经 mc-service 转发，避免 server 成视频瓶颈）；WebRTC 转发与远控体验由 R-MC-004 / mc-sfu 承担 |
| 动作统一封装 | ADB 动作（install / pressKey / reboot / screenshot）与无障碍动作（find / click / sendKeys / swipe）统一封装为 `runAction` 消息，由 `actionType` 字段区分通道；对 R-MC-005 网关透明 |

**异常场景**：Agent 启动鉴权失败（agentKey 无效）则拒绝连接并记录；Agent 心跳超时（默认 90s）判离线、关闭 WS、推 terminal_offline 事件；动作下发时 Agent 离线则返回失败并告警；动作执行超时（默认 30s）返回失败；占用锁冲突（设备被其他会话占用）返回 `locked` 失败原因；scrcpy 启动失败则远控降级为截图轮询（R-MC-004 兜底）；Agent 断线后指数退避自动重连（1s/2s/5s/10s/30s 封顶），重连后重新走鉴权与设备清单上报。

**验收标准**：Agent 启动可经 II-02 连中心侧编排器并完成鉴权与设备清单上报；Agent 心跳可被中心侧识别并维护在线状态，超时可判离线并通知 R-MC-001 台账；真机动作经 R-MC-005 网关可路由到本模块、经 II-02 下发 Agent 落地（ADB install/pressKey/reboot/screenshot 与无障碍 find/click/sendKeys/swipe）；远控打开时 Agent 可启动 scrcpy 推流到 mc-sfu 端口、浏览器经 mc-sfu WebRTC 看到画面；占用锁可保证同一台设备同时只有一个会话使用；Agent 断线可自动重连。

**关联接口**：II-02 Agent 长连接协议（本需求为提供方）、II-03 远控画面传输（与 R-MC-004 协商）、II-07 执行网关动作接口（本需求为 RealDeviceSdk 实现方）。

**V1.2 待补充能力**（MVP 不实现，预留）：iOS 设备控制（WebDriverAgent + sonic-ios-bridge，需 Mac + Xcode）、设备热插拔（ddmlib `AndroidDeviceStatusListener`）、设备保护（温升 / 低电 / 存储满自动隔离）、远程升级（Agent OTA）、分布式占用锁（多 mc-service 副本场景）。

> 说明：MC 子系统的用户需求均已按"目标澄清 → 梳理场景 → 理清流程 → 理清流程-异常 → 列出规则"五步分析，并按"足够小、端到端、独立性、可测性"四原则重新划分为 R-MC-001 ~ R-MC-015 共 15 个需求（V4.1 / V3.10 新增 R-MC-015 真机 Agent 运行时与编排，原 8 项用户需求拆分不变）。
