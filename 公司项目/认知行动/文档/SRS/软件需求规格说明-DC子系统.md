...
本文件为「认知行动平台」数据爬取子系统（DC，DC（Data Crawler））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.9）按子系统拆分的组成部分，承载 数据爬取子系统 的全部功能需求（R-DC-001 ~ R-DC-008（8 项））。需求编号 `R-DC-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
