...
本文件为「认知行动平台」运维监控子系统（OM，OM（Ops Monitor））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V4.3）按子系统拆分的组成部分，承载 运维监控子系统 的全部功能需求（R-OM-001 ~ R-OM-002（2 项））。需求编号 `R-OM-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。

> 版本演进说明（V3.9 / ADR-004，2026-07-17）：原 OM 前端运维监控页（OM-01~05）已删除，平台可观测性交由 KubeSphere 底座原生控制台承担；OM 后端作为分析入仓 + II-04/II-11 上报接口消费方留存。R-OM-001/002 需求编号与内容**全程不变**（详见各需求块下的 V3.9 加注）。
...
| 《ADR-004-删除OM运维监控并独立COM为系统管理控制台》 | V3.9 加注依据：删 OM 前端运维页、COM 独立、OM 后端留存 |
| 《软件需求跟踪矩阵.xlsx》 | 需求双向追溯唯一权威源（`R-OM-NNN` 追溯链） |
| 《软件概要设计说明》V3.2 | OM 子系统功能模块设计（M-OM-NN） |
| 《软件详细设计说明-OM子系统》V1.2 | OM 子系统详细设计（V1.2 删前端观测，保留后端入仓） |
...
> V3.9（ADR-004）：前端运维页 OM-01~05 已删，可观测交 KubeSphere。需求保留，OM 后端作为分析入仓 + II-04/II-11 上报接口消费方留存。
...
> V3.9（ADR-004）：前端运维页 OM-01~05 已删，可观测交 KubeSphere。需求保留，OM 后端作为分析入仓 + II-04/II-11 上报接口消费方留存。原「只读观测 + 看板元数据镜像」的前端入口（日志检索 UI、指标看板 UI、告警观测 UI、运维作业）改由运维人员直连 KubeSphere 原生控制台（OpenSearch / Grafana / Alertmanager）完成；OM 仅保留告警事件归档（webhook 落库）。需求 R-OM-002 编号与内容不变。
