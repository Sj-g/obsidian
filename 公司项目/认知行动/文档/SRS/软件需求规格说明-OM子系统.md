...
本文件为「认知行动平台」运维监控子系统（OM，OM（Ops Monitor））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.8）按子系统拆分的组成部分，承载 运维监控子系统 的全部功能需求（R-OM-001 ~ R-OM-002（2 项））。需求编号 `R-OM-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
