...
本文件为「认知行动平台」蜂群智能体子系统（SWM，SWM（Swarm））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V4.3）按子系统拆分的组成部分，承载 蜂群智能体子系统 的全部功能需求（R-SWM-001 ~ R-SWM-005（5 项））。需求编号 `R-SWM-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
...
| 《软件概要设计说明》V3.2 | SWM 子系统功能模块设计（M-SWM-NN） |
...
**用户需求概述**：管理驱动智能体作业的提示词，支持提示词模板库分类、版本管理与迭代优化，并提供 Playground 试运行调试与版本输出对比能力以加速提示词的编写与调优，最终以 agent_id（模板共性）+ account_id（动态实例个性）组合为标识将完整的提示词包（提示词模板、账号级动态提示词实例、作业策略）同步至 MC 持存。

**第①步 目标澄清**（工具：干系人多角度分析 ｜ 产物：子目标清单 ｜ 验收：是否穷举所有干系人的价值诉求）：①提示词工程师能管理提示词模板库（分类/版本/迭代）；②提示词工程师能在 Playground 中试运行调试提示词并对比不同版本的输出；③提示词包能以 agent_id+account_id 组合同步至 MC 持存（模板按 agent_id、动态实例按 account_id）。
...
| 系统（自动） | 按 MC 作业效果反馈迭代、以 agent_id+account_id 组合同步提示词包至 MC（II-14，模板按 agent_id、动态实例按 account_id） |
...
| 定义同步（以 agent_id+account_id 组合同步 MC，II-14） | 提示词包以 agent_id+account_id 组合同步至 MC | ③定义同步 |
...
**需求目的**：管理驱动智能体作业的提示词，支持提示词模板库分类、版本管理、Playground 试运行调试、版本输出对比与迭代优化，并以 agent_id（模板共性）+ account_id（动态实例个性）组合为标识将完整的提示词包（提示词模板、账号级动态提示词实例、作业策略）同步至 MC 持存。账号主数据由 MC 统一维护（R-MC-011）。提示词的运行时场景适配与动态调用（按场景选用模板、运行时填充生成指令）归 MC 的 agent-runtime 子模块承担（见 R-MC-006/014），不在本子系统。
...
| 版本管理 | 系统对提示词模板进行版本管理，以 agent_id 为主键索引（模板为智能体共性），支持版本对比、回滚与变更记录；账号级动态提示词实例按 account_id 1:1 归属，单独版本管理 |
...
| 提示词包同步 | 系统将 agent_id 对应的提示词模板、作业策略组装为提示词包，并为该 agent_id 绑定的每个 account_id 生成账号级动态提示词实例，经 II-14 接口以 agent_id+account_id 组合为主键同步至 MC 持存（见 R-MC-014）；记忆不在提示词包内，由 MC 的 agent-runtime 运行时按 account_id 向 SWM 记忆服务读取；提示词更新时以同一 agent_id+account_id 组合重新同步并升级版本号 |

**异常场景**：版本回滚失败时告警；Playground 试运行推理调用 IRS 失败时展示错误并支持重试，不影响线上提示词包；效果反馈缺失时暂停迭代；同步至 MC 失败时暂存重试。

**验收标准**：提示词模板库可分类管理；版本可按 agent_id 对比回滚（账号级动态实例按 account_id）；可在 Playground 中填入提示词草稿即时调用 IRS 试运行并查看产出；可将不同版本提示词并排对比输出；可结合反馈与调优结果迭代优化；提示词包可以 agent_id+account_id 组合同步至 MC 持存（模板按 agent_id、动态实例按 account_id）。
...
**需求目的**：实现智能体作业记忆的存储、场景沉淀、历史复用与个性化状态延续，使智能体长期行为符合行为风格且不断演进。记忆按 account_id 1:1 归属——同一 agent_id 下绑定的不同账号各自拥有独立的记忆（账号个性），互不影响；一个 agent_id 可绑多个账号时，每个账号的记忆独立存储、独立复用。
...
| 规则 | 说明 |
|------|------|
| 记忆存储 | 系统对智能体作业过程中的关键信息进行记忆存储，以 account_id 为主键索引（账号级记忆 1:1 归属），包括已完成任务、互动对象、立场表达等；同一 agent_id 下不同账号的记忆相互独立 |
...
**验收标准**：作业关键信息可按 account_id 记忆存储（账号级 1:1 归属）；重复场景经验可沉淀为模式；相似作业可复用历史记忆；智能体可基于账号级记忆维持个性化延续；启用记忆后智能体的行为风格一致性有效性由评测子系统（R-SWM-004）的 loop 评估器度量，行为风格一致性评分须达到 loop 评估器设定的可用线，且跨轮次作业的行为连贯性不退化。

**关联接口**：无（记忆为 SWM 内部存储）。

### 智能体构建

**用户需求概述**：根据提示词构建可作业的智能体——编写驱动智能体作业的提示词模板与记忆初值、配置作业策略，形成完整的智能体定义并以 agent_id（模板）+ account_id（动态实例）组合为标识，经 II-14 同步至 MC 持存、供 OCC 行动编排引用。

**第①步 目标澄清**（工具：干系人多角度分析 ｜ 产物：子目标清单 ｜ 验收：是否穷举所有干系人的价值诉求）：①构建侧能按提示词组装出完整智能体定义并以 agent_id 标识；②定义能与 account_id 1:N 绑定（一个 agent 绑多个账号，每账号独立动态提示词实例）并同步至 MC、作为 OCC 节点来源。
**第②步 梳理场景**（工具：用例图 ｜ 产物：场景 ｜ 验收：是否穷举所有用户操作）
...
| 账号运营人员 | 配置提示词模板与 account_id 1:N 绑定（一个 agent 绑多个账号） |
| 系统（自动） | 分配 agent_id、为每个绑定 account_id 生成动态提示词实例、以 agent_id+account_id 组合同步 MC（II-14）、作为 OCC 节点来源 |
...
flowchart LR
  ...
  subgraph 同步域["同步域：标识绑定与分发"]
    A3((系统-构建))
    UC3[系统:分配agent_id]
    UC4[系统:1:N绑定account_id并生成动态实例]
    UC5[系统:agent_id+account_id组合同步MC(II-14)]
    UC6[系统:作为OCC节点来源]
    A3 --> UC3 & UC4 & UC5 & UC6
    UC1 -. use .-> UC3
    UC2 -. use .-> UC4
...
sequenceDiagram
  ...
  工程师->>SWM: 按提示词组装智能体定义
  运营->>SWM: 配置提示词模板与account_id绑定(可多个账号)
  SWM->>SWM: 分配agent_id并1:N绑定account_id、每账号生成动态提示词实例
  SWM->>MC: 以agent_id+account_id组合同步提示词包(II-14)
...
|--------|---------|------|
| SWM（定义组装） | 定义冲突 | 人工裁定 |
| SWM ↔ MC（1:N 绑定） | 同一 account_id 重复绑定不同 agent_id | 告警 |
...
| agent_id ↔ account_id 1:N 绑定（一个 agent 绑多个账号，每账号独立动态实例） | 一个 agent_id 可对应多个 account_id | ②绑定 |
| 以 agent_id+account_id 组合同步 MC（II-14） | 定义可经 II-14 同步至 MC 持存 | ②同步 MC |
...
**需求目的**：根据提示词（人设属性直接写进提示词文本）构建可作业的智能体——配置驱动作业的提示词模板与作业策略（目标、场景、边界），组装为提示词包（提示词模板、账号级动态提示词实例、作业策略）并以 agent_id（模板共性）+ account_id（动态实例个性）组合为标识，经内部接口 II-14 同步至群控子系统（MC）持存、供社信行动子系统（OCC）行动编排引用。一个 agent_id 可绑定 N 个 account_id，每个账号拥有独立的动态提示词实例。记忆初值不在提示词包内，由 SWM 记忆服务（R-SWM-002，按 account_id 索引）独立承载。本功能确立"SWM 生成提示词包、MC 持存并运行（agent-runtime）、OCC 编排引用"的分工。

**使用角色**：提示词工程师与账号运营人员（构建与配置）、系统（分配 agent_id 与同步）。

**前置条件**：提示词模板已建立（R-SWM-001）；作业策略规范已制定；MC 账号服务接口与 II-14 可用。

**输入**：提示词模板、作业策略（目标、场景、边界）、绑定的账号标识（account_id，可多个）。

**输出**：提示词包（以 agent_id+account_id 组合为标识，含模板与账号级动态实例）、同步确认与 MC 持存版本号。

**业务规则**：

| 规则 | 说明 |
|------|------|
| agent_id 形态 | agent_id 为稳定的逻辑标识，不随提示词重新生成而变；提示词模板版本作为 agent_id 下的子属性（智能体共性），每次提示词模板重新生成升级版本号；agent_id 与 account_id 为 1:N 绑定（一个智能体可绑多个账号），每个 account_id 拥有独立的动态提示词实例（账号个性）；绑定关系记录由 MC 的 R-MC-013 维护，本子系统在构建时引用 account_id |
| 提示词包组装 | 系统按提示词配置提示词模板与作业策略，组装为提示词包，以全局唯一的 agent_id 标识模板、以 agent_id+account_id 组合标识账号级动态实例；提示词模板可手动编写或由 R-SWM-005 对话编写生成，动态实例由模板结合账号属性生成 |
| 提示词包同步 | 构建完成的提示词包经内部接口 II-14 以 agent_id+account_id 组合为主键同步至 MC 持存，作为 MC 的 agent-runtime 子模块在任务执行时运行智能体的依据；同步成功率不低于 99% |
| OCC 节点来源 | 构建出的提示词包是 OCC 行动编排中智能体节点的定义来源（OCC 编排 MC 中已持存的智能体） |

**异常场景**：提示词包内部冲突（提示词与作业策略不一致）时提示人工裁定；与 account_id 绑定冲突（同一 account_id 重复绑定不同 agent_id）时告警；同步失败时保留上一可用提示词包并重试。

**验收标准**：可按提示词构建提示词包并以 agent_id（模板）+ account_id（动态实例）组合为标识；提示词包须包含提示词模板、作业策略两项必填要素，缺一不可构建；agent_id 为稳定逻辑标识、提示词模板版本可随重生成升级；可与 account_id 1:N 绑定（一个 agent 绑多个账号，每账号独立动态实例）；提示词包可经 II-14 以 agent_id+account_id 组合同步至 MC 持存并取得版本号且同步成功率不低于 99%；构建出的智能体可作为 OCC 行动编排的智能体节点来源。
...
**需求目的**：对构建出的智能体（R-SWM-003）进行质量评测，采用「线上效果 + 离线数据集」双轨评测范式评估智能体的有效性、稳定性与合规性；离线评测以"评测集、评估器、实验"三类可管理实体组织——评测集承载测试用例，评估器承载可复用的打分逻辑，实验承载一次可追踪、可对比的评测运行，淘汰低效定义、迭代优化高价值定义，形成"构建→评测→迭代"的闭环，使智能体定义随作业实践持续演进。双轨评测互补印证：线上效果反映真实行动表现，离线数据集提供受控、可复现、可对比的质量基准，避免单一维度误判。
...
| 规则 | 说明 |
|------|------|
| 线上有效性评测 | 系统基于 MC 可观测性（R-MC-010）上报的作业效果数据评测智能体的有效性（触达、互动、任务完成率等），反映真实行动表现 |
...
**关联接口**：EI-05 本地大模型推理（离线跑题）、II-10 行动编排下发与执行回传（MC 作业效果数据来源）。
...
sequenceDiagram
  ...
  participant IRS as IRS本地大模型(EI-05)
  工程师->>SWM: 上传画像素材文件+描述需求(指定agent_id编写模板)
  SWM->>SWM: 解析上传素材
...
| 版本化与回写 | 编写的提示词模板纳入版本管理（以 agent_id 索引，模板为智能体共性），回写至提示词库；账号级动态提示词实例由 R-SWM-003 智能体构建时基于模板结合账号属性生成；可由提示词工程师审核微调后纳入 R-SWM-003 智能体构建 |
