...
本文件为「认知行动平台」公共功能（COM，COM（Common））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V4.3）按子系统拆分的组成部分，承载 公共功能 的全部功能需求（R-COM-001（1 项））。需求编号 `R-COM-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
...
| 《软件概要设计说明》V3.2 | COM 子系统功能模块设计（M-COM-NN） |
