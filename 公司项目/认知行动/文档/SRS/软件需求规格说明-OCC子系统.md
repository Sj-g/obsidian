...
本文件为「认知行动平台」社信行动业务子系统（OCC，OCC（Opinion Combat Command））的**软件需求规格说明分册**。它是《软件需求规格说明》总册（V3.8）按子系统拆分的组成部分，承载 社信行动业务子系统 的全部功能需求（R-OCC-001 ~ R-OCC-009（9 项））。需求编号 `R-OCC-NNN` 与总册一致，全程不变，作为需求双向追溯的依据。
